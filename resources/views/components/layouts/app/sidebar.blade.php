    ...
    <flux:sidebar sticky stashable class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <a href="{{ route('dashboard') }}" class="me-5 flex items-center space-x-2 rtl:space-x-reverse" wire:navigate>
            ...
            <span class="truncate text-xs text-gray-400">
                {{ strtoupper(auth()->user()->role->name ?? 'Tanpa Role') }}
            </span>
        </a>
        ...
        <flux:navlist variant="outline">
            <flux:navlist.group :heading="__('Platform')" class="grid">
                ...
                @if (auth()->user()->role->name === 'admin')
                    <flux:navlist.item class="mb-2" icon="users" :href="route('users.index')"
                        :current="request()->routeIs('users.index')" wire:navigate>
                        {{ __('User') }}
                    </flux:navlist.item>
                    <flux:navlist.item class="mb-2" icon="users" :href="route('harga-tindakan.index')"
                ...
                @if (auth()->user()->role->name === 'petugas_pendaftaran')
                    <flux:navlist.item class="mb-2" icon="user-plus" :href="route('patients.index')"
                        :current="request()->routeIs('patients.*')" wire:navigate>
                        {{ __('Pendaftaran Pasien') }}
                    </flux:navlist.item>
                @endif

                {{-- Menu Tindakan Medis - hanya untuk dokter --}}
                @if (auth()->user()->role->name === 'dokter')
                    <flux:navlist.item class="mb-2" icon="user" :href="route('MedicalActions.index')"
                        :current="request()->routeIs('MedicalActions.*')" wire:navigate>
                        {{ __('Tindakan Medis') }}
                    </flux:navlist.item>
                @endif
